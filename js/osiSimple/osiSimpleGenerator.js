  ...
  $col3.appendChild(mkeEl('b',['interactive-osi-model__heading','flex','flex--center'],'Description'))
    ...
    newArr.forEach((el,idx,arr)=>{
      if(el.layerNum){
        ...
        $layer.onclick=(e)=> handleItemClick(e,$container)

        let $description=mkeEl('div',['interactive-osi-model__description',"description",'flex','flex--column','flex--center','hidden']);
        // let $p=mkeEl('p',[],`${el.description}`)
        let $vid=mkeEl('embed',['float--left']);
          $vid.src=el.url;
          // $description.appendChild($p)
          $description.appendChild($vid);
        ...
        $pdu.dataset.id=idx;
        $pdu.onclick=(e)=>handleItemClick(e,$container);
